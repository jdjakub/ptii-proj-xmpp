...
\lstnewenvironment{ocaml}{\lstset{language=[Objective]Caml, basicstyle=\ttfamily}}{}
...
\begin{itemize}
  ...
  \item Testing utility function \code{begin_transcription}. Spawns \emph{transcriber} thread, which repeatedly calls \code{#spill} on the client and logs the received XML in \code{<clientname>.xml} somewhere in the \code{transcripts/} directory tree. On error, transcriber sets boolean `finished' flag that user should use to abort early, and quits. User can set `finished' to cause the transcriber to quit, if desired. \code{with_client} returns the action thread.
\end{itemize}

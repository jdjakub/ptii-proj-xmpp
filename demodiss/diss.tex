...
Word Count:         & \bf 8464\footnotemark[1]  \\
...
To use the OCaml language and its libraries to build a performant server implementing the XMPP protocol, conforming to the core XMPP specification as well as the XMPP-IM extension specification.
...
\begin{enumerate}
  ...
  \item Respond with `success' to client. Restart the stream anew by handshaking again\footnote{This is mainly intended for encrypted connections and suchlike, so that the entire stream can be considered secured; nevertheless, it is still required in other cases.}. Offer features again, this time consisting of the required process of \emph{resource binding}.
...
\begin{itemize}
  ...
  Handshaking with other servers and forwarding to clients residing on them---the \emph{federated} model---complicates the process, as the server must be able to receive and understand the language of XMPP servers as well as clients. This would include negotiations with other servers on their terms---possibly requiring implementation of encryption and authentication, or other stream features, in order to conform. Forwarding messages to \emph{directly connected} clients is the priority.
...
I view supporting general CRUD (Create, Read, Update, Delete) operations for the roster as extension work, since implementing them before getting to stanza delivery would be too time consuming; of the CRUD operations, only Reading rosters will apply. The rosters will therefore be determined by the content of the roster files, and I will manually ensure their consistency. Since they do not change during runtime or get updated by the server, this consistency is maintained.
...
They both make heavy use of the \code{Xml} and \code{Xmpp} modules.
...
The fact that an XML node can be simple text is accounted for, by assuming text content when an attempt to parse a tag fails.
...
\begin{xml}[label={lst:xmlsample}, caption={XML sample.}]
...
For the purposes of output, I committed to using only one, standard prefix per namespace (recall that prefixes are local to parts of a tree, but such freedom is not really needed here). I bundled namespaces and prefixes together, so that I could refer to them by a simple OCaml identifier. For example, the opening \code{<stream:stream>} element uses the \code{stream:} prefix, referring to the namespace \code{http://etherx.jabber.org/streams}. I made definitions (in the \code{Xmpp} module) like so:
...
\chapter{Conclusions}
This project has demonstrated that it is possible to create a performant message-forwarding server using the OCaml language and libraries. It has also demonstrated some of the benefits of using a language like OCaml for parsing-related tasks. The basic implementation I have made can function as a substrate for more advanced XMPP functionality.
...
\chapter{Presence logs}\label{apdx:pres-logs}
\scriptsize\verbatiminput{../prestests.txt}

\chapter{Throughput graphs}\label{apdx:graphs}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{../transcripts/lipsum/5n5/cold/cold.png}
\end{figure}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{../transcripts/lipsum/10n10/cold/cold.png}
\end{figure}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{../transcripts/lipsum/20n20/cold/cold.png}
\end{figure}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{../transcripts/lipsum/30n30/cold/cold.png}
\end{figure}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{../transcripts/lipsum/40n40/cold/cold.png}
\end{figure}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{../transcripts/lipsum/60n60/cold/cold.png}
\end{figure}

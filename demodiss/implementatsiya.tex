...
The rest of the setup implements the points in Section~\ref{sec:prep-handshake}. The server offers the list of stream features to negotiate, consisting solely of the \code{PLAIN} authentication mechanism. After client chooses \code{PLAIN}, login information is extracted from the \code{base64}-encoded text with a special Angstrom parser.
...
Finally, the initial presence is received, echoed and forwarded to the user's subscribers. If universally subscribed, the server obtains list of currently online clients from the \code{Dispatch} module, filtering out the originator from the list before forwarding as the presence was already echoed back.

At this point, setup is finished, now moves into servicing incoming stanzas.

\subsubsection{Service stage}
The server now calls \code{Dispatch.client_connected} to obtain a work queue. It sets up a worker thread, which loops, extracting from the queue and forwarding the stanzas to the client.

Next, the main thread itself enters a loop, expecting stanzas and behaving accordingly. Auxiliary behaviour such as parsing the stanzas and handling errors lives in the loop body, while the main behaviour is separated into functions \code{handle_iq}, \code{handle_presence}, \code{handle_message}:

\begin{itemize}
  \item \code{handle_iq} returns an \code{Iq.t} representing the stanza error \code{feature-not-implemented}. The calling code translates this into the actual XML and pushes this to the work queue.
...
\begin{itemize}
  ...
  \item \code{#handshake} method performs handshake and returns the roster. The handshake here is designed to be symmetric to server's, to ensure it all goes smoothly. It is not designed to anticipate substantial variation in the server responses, as it is purely for testing.
